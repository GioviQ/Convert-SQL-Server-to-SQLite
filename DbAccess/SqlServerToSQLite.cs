    ...
    public class SqlServerToSQLite
    {
        ...
        private static DbType GetDbTypeOfColumn(ColumnSchema cs)
        {
            ...
            if (cs.ColumnType == "numeric")
                return DbType.Double;
            if (cs.ColumnType == "timestamp" || cs.ColumnType == "datetime" || cs.ColumnType == "datetime2" || cs.ColumnType == "date")
            ...
            SqlTableSelectionHandler selectionHandler)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        if (reader["TABLE_NAME"] == DBNull.Value || (string)reader["TABLE_NAME"] == "sysdiagrams")
                            continue;
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                while (reader.Read())
                {
                    ...
                    if (dataType == "timestamp")
                        dataType = "blob";
                    else if (dataType == "datetime" || dataType == "smalldatetime" || dataType == "date" || dataType == "datetime2")
                        dataType = "datetime";
